...
- **Post-play submission prompt (2026-04-17):** live via `qf-dapp/games/qf-leaderboard-prompt.js` shared helper. Wired across 21 games (7 submit-freeplay-only, 7 mixed-path free-play branches, 7 evaluate-path with `!leagueMode && !duelMode` guards). Battleships and maffsy skipped — their custom submit paths (`/battleships/:code/shoot`, `/maffsy/complete`) don't create `active_game_state` rows; separate backend task needed. Helper does 3 parallel eligibility calls (daily/weekly/monthly), renders Sabre-themed modal only when at least one period qualifies, dedups per sessionId via sessionStorage, POSTs via JWT (no wallet signing). Second-chance submit on My Account High Scores remains as a fallback path.
