...
- **Global leaderboard entry subsidised by escrow (revenue leak).** `/global-leaderboard/enter` calls `sc.splitFee({ value: 50 QF })` where the signer is the server's escrow wallet — 50 QF is burned/teamed from escrow funds, not from the user. `req.body.txHash` is accepted but never required, never validated, stored as NULL. User pays nothing; MetaMask never prompts. Realised cost so far: 1 row — Jon's `global_leaderboard_entries.id=1` (sudoku-duel, 3629, 2026-04-18). Fix design agreed: Option 1 (trust-the-hash mirror of mint flow) — client signs 50 QF to escrow, server requires `txHash`, `logIncoming('leaderboard-fee', ...)` before existing splitFee+INSERT. Queued; pre-flight complete (helper's sendTransaction pattern, `/achievement/mint-config` blueprint, `escrow.mjs` `verifyPaymentTx` PARKED confirmed); implementation not started this session.
- **My Account second-chance Submit has the same revenue leak.** `submitToLeaderboard()` in `qf-dapp/my-account/index.html:688` POSTs to the same unprotected endpoint. No user-side payment triggered. Fix follows Option 1 for `/global-leaderboard/enter` — queue as the same task or immediate follow-up.
